...
func main() {
	...
	toStore := flag.String("input", "deadbeef", "Input to store in memory, either efficiently or not")
...
func comparePageTableProcesses(input *InputData) {
	...
	var key string
	fmt.Printf("Running Mixed Page Table processes for input '%s' in batches of %d\n", input.toStore, input.batchSize)
	key = fmt.Sprintf("mixed-page-table-%s", input.toStore)
	for i := 0; i < input.procs; i++ {
		...
		wg.Add(1)
		if i%2 == 0 {
			go stat.RunInsanePageTableProcess(key, input.toStore, &wg)
		} else {
			go stat.RunSanePageTableProcess(key, input.toStore, &wg)
		}
	}
	wg.Wait()
	fmt.Printf("Finished running processes, took %s\n", time.Since(start))
	path := fmt.Sprintf("lab-part-2-length-%d-step-%d-proc-%d-batches-%d.json", len(input.toStore), input.stepSize, input.procs, input.batchSize)
	err = stat.Dump(path)
	if err != nil {
...
func (d StatsData) RunTimedProcess(path string, time int) (map[string]string, error) {
	schedStats := make(map[string]string, 10)
	strTime := strconv.Itoa(time)
	cmd := exec.Command(path, "-time", strTime)
	var output bytes.Buffer
	cmd.Stdout = &output
	cmd.Stderr = &output
	err := cmd.Run()
	if err != nil {
		return schedStats, fmt.Errorf("Failed to run %s for time %d: error:%s, output:%s\n", path, time, err, output.String())
	}
	err = json.Unmarshal(output.Bytes(), &schedStats)
	if err != nil {
		return schedStats, fmt.Errorf("Failed to parse stats:%s, output:%s\n", err, output.String())
	}
	return schedStats, nil
}

func (d StatsData) RunInputProcess(path string, input string) (map[string]string, error) {
	schedStats := make(map[string]string, 10)
	cmd := exec.Command(path, "-input", input)
	var output bytes.Buffer
	cmd.Stdout = &output
	cmd.Stderr = &output
	err := cmd.Run()
	if err != nil {
		return schedStats, fmt.Errorf("Failed to run %s process for input %s: error:%s, output:%s\n", path, input, err, output.String())
	}
	err = json.Unmarshal(output.Bytes(), &schedStats)
	if err != nil {
		return schedStats, fmt.Errorf("Failed to parse stats:%s, output:%s\n", err, output.String())
...
func (d *StatsData) RunInsanePageTableProcess(key, input string, wg *sync.WaitGroup) {
	defer wg.Done()
	schedStats, err := d.RunInputProcess("./insane_page_table", input)
	if err != nil {
...
func (d *StatsData) RunSanePageTableProcess(key, input string, wg *sync.WaitGroup) {
	defer wg.Done()
	schedStats, err := d.RunInputProcess("./sane_page_table", input)
	if err != nil {
...
func (d *StatsData) RunIOProcess(key string, time int, wg *sync.WaitGroup) {
	defer wg.Done()
	schedStats, err := d.RunTimedProcess("./io_intensive", time)
	if err != nil {
...
func (d *StatsData) RunCPUProcess(key string, time int, wg *sync.WaitGroup) {
	defer wg.Done()
	schedStats, err := d.RunTimedProcess("./cpu_intensive", time)
	if err != nil {
