...
func main() {
	...
	for runTime := *stepSize; runTime <= *maxTime; runTime += *stepSize {
		runStart := time.Now()
		if *phase == PHASE_ALL || *phase == PHASE_IO {
		...
		if *phase == PHASE_ALL || *phase == PHASE_CPU {
		...
		if *phase == PHASE_ALL || *phase == PHASE_MIXED {
	...
	path := fmt.Sprintf("lab-part-1-max-%d-step-%d-proc-%d-batches-%d.json", *maxTime, *stepSize, *procs, *batchSize)
	err = stat.Dump(path)
	if err != nil {
